...
export async function blockDates(
  ...
  if (reservation) {
    var dates = [];
    var start = moment.utc(reservation.checkIn).toDate();
    var end = moment.utc(reservation.checkOut).toDate();
    start = new Date(start);
    end = new Date(end);
    var copy = new Date(start);
    let endTimevalue = end.setDate(end.getDate());
    let endTime = new Date(endTimevalue);
    dates.push(copy);

    let formateStartDate = moment(start).format('DD-MM-YYYY');
    let formateEndDate = moment(end).format("DD-MM-YYYY");
