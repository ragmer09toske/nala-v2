...
class FutureTransactionItem extends Component {
	...
	render() {
		...
		let date = data.checkIn != null ? moment(data.createdAt).format('DD-MM-YYYY') : 'Pending';
		...
		if (payoutAmount) {
			totalAmount = data && data.cancellationDetails && data.cancellationDetails.payoutToHost ? data.cancellationDetails.payoutToHost : totalAmount;
		}
